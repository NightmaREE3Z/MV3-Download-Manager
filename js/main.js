...
// Establish persistent connection with service worker
const port = chrome.runtime.connect({name: 'popup'});

// Service worker connection check
async function waitForServiceWorker(maxAttempts = 5) {
...
async function initializePopup() {
    try {
        const serviceWorkerReady = await waitForServiceWorker();
        ...
        // Load locale and render downloads
        if (typeof setLocale === "function") {
            const storedLocale = await LocaleStorage.get();
            await setLocale(storedLocale);
        }

        await render();
    } catch (e) {
        showPopupError("Failed to initialize: " + e.message);
    }
}
...
const LocaleStorage = {
    async get() {
        try {
            ...
            return result.selectedLocale || navigator.language.split('-')[0] || 'en';
        } catch (e) {
...
async function render() {
    ...
    try {
        ...
        showPopupError("Failed to contact background script: " + err.message);
    }
}
...
on($("#language-switch"), "change", async function() {
    try {
        ...
        showPopupError("Failed to set locale: " + e.message);
    }
});
